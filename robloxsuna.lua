...
-- 修复：透视距离被游戏重置问题 — 每帧强制重写 MaxDistance
...
local Config = {
    ...
    ShowJob = true,
    ShowDistance = true,
    MaxDistance = 0, -- 修复：0 = 无限距离（Roblox 官方约定）
    UpdateRate = 0.15,
    TeamCheck = false,
}
...
-- 修复：降频更新循环 + 有效性校验 + 每帧强制重写 MaxDistance
    ...
    while task.wait(Config.UpdateRate) do
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            ...
            if ESPList[p.UserId] then
                ...
                -- ★ 核心修复：每帧强制重写 MaxDistance，防止游戏反作弊脚本重置为低值
                if ESPList[p.UserId].Billboard.MaxDistance ~= Config.MaxDistance then
                    ESPList[p.UserId].Billboard.MaxDistance = Config.MaxDistance
                end
            end
        end
    end
...
ScaleBtnCorner.CornerRadius = UDim2.new(0, 8)
